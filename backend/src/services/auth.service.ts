...
export class AuthService {
   ...
   */
  static async authenticateUser(credentials: LoginCredentials, ipAddress?: string, userAgent?: string): Promise<AuthResult> {
    try {
      ...
      if (isLockedOut) {
        await this.logAuditEvent(null, 'LOGIN_ATTEMPT_BLOCKED', { email, reason: 'account_locked', ipAddress });
        return {
          success: false,
      ...
      if (!user) {
        ...
        await this.logAuditEvent(null, 'LOGIN_FAILED', { email, reason: 'user_not_found', ipAddress });
        return {
          success: false,
      ...
      if (!isPasswordValid) {
        await this.incrementLoginAttempts(email);
        await this.logAuditEvent(user._id.toString(), 'LOGIN_FAILED', { email, reason: 'invalid_password', ipAddress });
        return {
          success: false,
          error: 'Invalid credentials'
        };
      ...
      if (user.twoFAEnabled) {
        ...
        if (!isTotpValid) {
          await this.incrementLoginAttempts(email);
          await this.logAuditEvent(user._id.toString(), 'LOGIN_FAILED', { email, reason: 'invalid_2fa', ipAddress });
          return {
            success: false,
      ...
      await this.logAuditEvent(user._id.toString(), 'LOGIN_SUCCESS', { email, ipAddress, userAgent });
