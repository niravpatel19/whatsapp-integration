...
import { MessageType, MessageStatus, VALIDATION_PATTERNS, FIELD_LIMITS } from '../types/database.types';
...
export interface IMessageModel extends Model<IMessage> {
  ...
  getMessageStatistics(userId: string, sessionId?: string): Promise<{
    total: number;
    sent: number;
    delivered: number;
    read: number;
  ...
  updateMessageStatus(messageId: string, status: MessageStatus, error?: string): Promise<IMessage | null>;
}
...
MessageSchema.statics.createMessage = async function(messageData: {
  ...
  if ([MessageType.IMAGE, MessageType.DOCUMENT, MessageType.AUDIO, MessageType.VIDEO, MessageType.STICKER].includes(messageData.type) && !messageData.mediaUrl) {
  ...
  const pipeline = [
    { $match: matchStage },
    {
      $group: {
        ...
        sent: {
          $sum: {
            $cond: [{ $in: ['$status', [MessageStatus.SENT, MessageStatus.DELIVERED, MessageStatus.READ]] }, 1, 0]
          }
        },
        delivered: {
          $sum: {
