...
export interface IQREventModel extends Model<IQREvent> {
  ...
  getQRStatistics(userId: string, sessionId?: string): Promise<{
...
QREventSchema.statics.incrementTries = async function(qrEventId: string): Promise<IQREvent | null> {
...
export const QREvent: IQREventModel = mongoose.model<IQREvent, IQREventModel>('QREvent', QREventSchema);