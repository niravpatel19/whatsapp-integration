...
export interface IAPIKeyModel extends Model<IAPIKey> {
  generateAPIKey(userId: string, options?: {
    label?: string;
    permissions?: APIKeyPermission[];
  ...
  rotateAPIKey(keyId: string, userId: string, gracePeriodHours?: number): Promise<{
    newKey: IAPIKey;
    rawKey: string;
    oldKey: IAPIKey;
  }>;
  getUserAPIKeys(userId: string, includeRevoked?: boolean): Promise<IAPIKey[]>;
  getUsageStatistics(keyId: string, userId: string): Promise<{
...
APIKeySchema.statics.getUserAPIKeys = async function(
  ...
  return await this.find(query)
    .sort({ createdAt: -1 })
    .select('-keyHash'); // Don't return the hash for security
};
