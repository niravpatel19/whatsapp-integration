...
import { SessionStatus, DeviceInfo, VALIDATION_PATTERNS, FIELD_LIMITS } from '../types/database.types';
...
SessionSchema.statics.getUserSessions = async function(
  ...
  return await this.find(query)
    .sort({ createdAt: -1 })
    .select('-wppState'); // Exclude WPP state for performance
};
    ...
    { $match: { userId: new Types.ObjectId(userId) } },
    {
      $group: {
        ...
        active: {
          $sum: {
            $cond: [
              { $nin: ['$status', [SessionStatus.EXPIRED, SessionStatus.ERROR]] },
              1,
              0
            ]
          }
        ...
        expired: {
          $sum: {
            $cond: [
              { $in: ['$status', [SessionStatus.EXPIRED, SessionStatus.ERROR]] },
              1,
              0
            ]
          }
...
export const Session: ISessionModel = mongoose.model<ISession, ISessionModel>('Session', SessionSchema);